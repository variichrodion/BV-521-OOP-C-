...
public:
    ...
    friend class FileSystem;                                     // FileSystem - дружественный класс
};
...
private:
    ...
    enum class Commands {                                        // класс-перечисление "команды"
        CD, LS, TREE
    };

public:
    FileSystem();                                                // конструктор FileSystem
    Folder* getCurrentFolder();                                  // получить указатель на текущую директорию
    void CommandProcessor(const std::string&);                   // обработчик команд

private:
    void initStruct();                                           // создание тестовой файловой системы
    void navigateToPath(const std::string&);                     // определяем метод перехода (полный или относительный путь)
    ...
    std::vector<std::string> splitStr(const std::string&, char); // разделение строки по символу
    Commands parseCommand(const std::string&);                   // преобразование string -> enum Commands
};